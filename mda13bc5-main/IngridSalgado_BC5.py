...
#Business Case 5 - Spotify Analytics Assistant

#Nombre: Ingrid Salgado
#Repositorio GitHub: [PEGAR_URL_GITHUB]
#App Streamlit: [PEGAR_URL_STREAMLIT]

#Descripción:
#Prototipo de asistente analítico conversacional sobre datos reales inspirado en el historial de escucha de Spotify.
#La aplicación permite formular preguntas en lenguaje natural y genera visualizaciones automáticas a partir del dataset.
...
#
#    Mi aplicación sigue una arquitectura text-to-code porque el usuario escribe una pregunta en lenguaje natural y el modelo no responde con texto libre, sino con código Python en formato JSON. El LLM recibe la estructura del dataframe, el significado de las columnas, las columnas derivadas que preparé (minutes_played, month_name, day_name, week_part, season, etc.) y las reglas del formato de salida. 
#    El LLM devuelve un JSON con tipo, codigo e interpretacion. Ese código se ejecuta localmente en la app de Streamlit sobre el dataframe df, no en el modelo. 
#    El LLM no recibe los datos directamente para mantener la arquitectura del caso: el análisis real ocurre en local y el modelo solo genera instrucciones analíticas, no accede al contenido completo del dataset.
#
#
...
#
#    Mi aplicación sigue una arquitectura text-to-code porque el usuario escribe una pregunta en lenguaje natural y el modelo no responde con texto libre, sino con código Python en formato JSON. 
#    El LLM recibe la estructura del dataframe, el significado de las columnas, las columnas derivadas que preparé (minutes_played, month_name, day_name, week_part, season, etc.) y las reglas del formato de salida. 
#    El LLM devuelve un JSON con tipo, codigo e interpretacion. 
#    Ese código se ejecuta localmente en la app de Streamlit sobre el dataframe df, no en el modelo. 
#    El LLM no recibe los datos directamente para mantener la arquitectura del caso: el análisis real ocurre en local y el modelo solo genera instrucciones analíticas, no accede al contenido completo del dataset.
#
#
# 3. EL FLUJO COMPLETO
...
#
#    Primero, el usuario escribe una pregunta en la interfaz de Streamlit.
#    La app ya tiene cargado el JSON en un dataframe y lo ha preparado antes con columnas derivadas para simplificar el análisis. 
#    Después, construyo un prompt dinámico con información real del dataset, como fechas mínimas y máximas y valores observados en columnas categóricas. 
#    Ese prompt se envía al modelo junto con la pregunta del usuario. 
#    El modelo devuelve un JSON con código Python e interpretación. 
#    La app parsea ese JSON, ejecuta el código localmente sobre df y espera obtener una figura de Plotly en la variable fig. 
#    Si fig existe, la app la muestra en pantalla, enseña la interpretación y también el código generado; si no, muestra un mensaje de error controlado.